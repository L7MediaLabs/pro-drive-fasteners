...
function LeadsPage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {STATUSES.map((s) => (
              <button
                ...
                style={{
                  ...
                  color: status === s ? "#0A0A08" : "var(--pdx-text-mute)",
                }}
              >
                {s}
              </button>
          ...
          {(["email", "website", "meta"] as const).map((k) => (
            <button
              ...
              style={{
                ...
                color: sig[k] ? YELLOW : "var(--pdx-text-mute)",
                cursor: "pointer",
      ...
      <div style={cardStyle}>
        <div style={{ ...mono, fontSize: 10, color: "var(--pdx-text-mute)", marginBottom: 12, letterSpacing: "0.15em" }}>
              ...
              {filtered.map((l: Lead) => (
                <Fragment key={l.company}>
                  <tr
                    ...
                    style={{ borderTop: "1px solid var(--pdx-border)", cursor: "pointer" }}
                  >
                    <td style={td}>
                      <div style={{ color: "var(--pdx-text)", fontFamily: "Assistant, sans-serif", fontWeight: 600, fontSize: 13 }}>{l.company}</div>
                      <div style={{ ...mono, fontSize: 9, color: "var(--pdx-text-faint)" }}>{l.location}</div>
                    </td>
                    <td style={td}><span style={{ ...mono, color: YELLOW, fontSize: 13 }}>{l.score}</span></td>
                    <td style={td}><span style={{ ...mono, fontSize: 10, color: "var(--pdx-text-dim)" }}>{l.status}</span></td>
                    <td style={td}><SignalBadges signals={l.signals} /></td>
                    <td style={td}><span style={{ ...mono, color: "var(--pdx-text)", fontSize: 12 }}>{l.websiteVisits}</span></td>
                    <td style={td}><span style={{ ...mono, fontSize: 11, color: "var(--pdx-text-dim)" }}>{l.topPage}</span></td>
                    <td style={td}><span style={{ ...mono, fontSize: 10, color: "var(--pdx-text-mute)" }}>{l.lastSeen}</span></td>
                    <td style={td}><UrgencyTag urgency={l.urgency} /></td>
                  </tr>
                  {expanded === l.company && (
                    <tr>
                      <td colSpan={8} style={{ background: "rgba(255,205,0,0.02)" }}>
                        <div style={{ borderLeft: `2px solid ${YELLOW}`, margin: "10px 0", padding: "8px 18px", fontFamily: "Assistant, sans-serif", fontSize: 13, color: "var(--pdx-text-dim)", lineHeight: 1.55 }}>
